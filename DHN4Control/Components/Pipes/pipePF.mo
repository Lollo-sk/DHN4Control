within DHN4Control.Components.Pipes;
...
model pipePF
  ...
  DHN4Control.Components.Pipes.BaseClass.DirectionalHeatLossPlugFlow outletHeatLosses(D = D, L = L, T_ext = T_ext, T_start = T_start, dIns = dIns, dWall = dWall, h = h, lambdaIns = lambdaIns, lambdaM = lambdaM, m_flow_nominal = m_flow_nominal, rhom = rhom, rhomcm = rhomcm)  annotation(
    Placement(visible = true, transformation(origin = {34, -2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  DHN4Control.Components.Pipes.BaseClass.PlugFlowCore plugFlowCore(D = D, L = L, T_ext = T_ext, T_start = T_start, cf = cf, dIns = dIns, dWall = dWall, h = h, lambdaIns = lambdaIns, lambdaM = lambdaM, m_flow_small = m_flow_small, rhom = rhom, rhomcm = rhomcm, u_nom = u_nom)  annotation(
    Placement(visible = true, transformation(origin = {-16, -2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  DHN4Control.Components.Pipes.BaseClass.DirectionalHeatLossPlugFlow inletHeatLosses(D = D, L = L, T_ext = T_ext, T_start = T_start, dIns = dIns, dWall = dWall, h = h, lambdaIns = lambdaIns, lambdaM = lambdaM, m_flow_nominal = m_flow_nominal, rhom = rhom, rhomcm = rhomcm)  annotation(
  ...
  DHN4Control.Components.Pipes.BaseClass.TimeDelayPlugFlow timeDelayPlugFlow(D = D, L = L, m_flow_nominal = m_flow_nominal, m_flow_start = m_flow_start)  annotation(
...
equation
  ...
  connect(massFlowSensor.M, timeDelayPlugFlow.m_flow) annotation(
    Line(points = {{-44, 7}, {-44, 27}, {-28, 27}}, color = {0, 0, 127}));
