...
partial model PartialInsulatedTube
  ...
  final parameter Modelica.Units.SI.ThermalConductance U_wm = lambdaM * 2 * Modelica.Constants.pi/ log((D / 2 + D / 2 + dWall) / D)"Thermal conductance water-metal per unit meter"; 
  final parameter Modelica.Units.SI.ThermalConductance U_me = lambdaIns * 2 * Modelica.Constants.pi/ log((D / 2 + dWall + dIns) / (D/2) + dWall)"Thermal conductance metal-ambient per unit meter";
